...
from reports_creator.utils import base_label, ReportValue, ReportRow
from reports_creator.highcharts import HighchartRender
from reports_creator.widgets import RangeField
from reports_creator.export_pdf import render_to_pdf
...
class ReportAdmin(object):
            ...
            try:
                ...
                if '__' in field:  # IF field has lookup
                    ...
                    for field_lookup in field.split("__"):
                        if not pre_field:
                            pre_field = base_model._meta.get_fields(include_parents=True, include_hidden=False)[0]
                    ...
                    if not 'self.' in field:
                        model_field = self.model._meta.get_fields(include_parents=True, include_hidden=False)[0]
                    else:
            ...
            if context_request.GET:
                ...
                if not context_request.GET.get('export', None) is None and not self.parent_report:
                    ...
                    if context_request.GET.get('export') == 'pdf':
                        ...
                        return render_to_pdf(self, 'reports_creator/export_pdf.html', context)
    ...
    def render(self, request, template_name, extra_context={}):
        context_or_response = self.get_render_context(request, extra_context)

        if isinstance(context_or_response, HttpResponse):
            return context_or_response
        return render(request=request, template_name=template_name, context=context_or_response)
                        ...
                        for field_lookup in k.split("__")[:-1]:
                            if pre_field:
                                if isinstance(pre_field, RelatedObject):
                                    ...
                                    base_model = pre_field.rel.to
                            pre_field = base_model._meta.get_fields(field_lookup)[0]

                        model_field = pre_field
                    else:
                        field_name = k.split("__")[0]
                        model_field = opts.get_fields(field_name)[0]
